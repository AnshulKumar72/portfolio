...
const Skills = () => {
  return (
    ...
    >
      ...
      {/* Skills Grid */}
      <div className="flex flex-wrap justify-between gap-y-10 gap-x-4 py-10">
            ...
            >
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 w-full">
                {category.skills.map((skill) => (
                  <div
                    key={skill.name}
                    className="flex items-center justify-center space-x-2 border-2 border-gray-700 rounded-3xl py-2 px-2 sm:py-2 sm:px-3 bg-transparent text-center"
                    aria-label={`Skill: ${skill.name}`}
                  >
                    <img
                      src={skill.logo}
                      alt={`${skill.name} logo` || "Skill logo"}
